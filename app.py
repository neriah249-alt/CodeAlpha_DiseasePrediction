#!/usr/bin/env python3
"""
app.py - VERSION AUTONOME (ne dépend pas de src/)
"""
...
def load_model_and_scaler():
    ...
    # Charger le modèle
    with open(os.path.join(base_path, 'models', 'best_model.pkl'), 'rb') as f:
...
try:
    ...
    st.sidebar.info("Vérifiez que models/best_model.pkl et models/scaler.pkl existent")
    model_ok = False

# Avertissement
st.info("⚠️ Application à titre démonstratif. Consultez toujours un médecin.")
...
if submit:
    if not model_ok:
        st.error("❌ Le modèle n'est pas chargé. Vérifiez les fichiers dans models/")
    else:
        # Créer DataFrame
        df = pd.DataFrame([{
        ...
        # STANDARDISER (obligatoire pour SVM)
